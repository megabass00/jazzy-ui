...
const clamp = (value: number, min = 0, max = 1): number => {
    if (process.env.NODE_ENV !== 'production') {
        if (value < min || value > max) {
            console.error(
                `Jazzy-UI: The value provided ${value} is out of range [${min}, ${max}].`
            );
        }
    }
 ...
 * @returns {object} - A Jazzy-UI color object: {type: string, values: number[]}
 */
export const decomposeColor = (color: JazzyColor | string): JazzyColor => {
    // Idempotent
    ...
    if (type === 'color') {
        ...
        if (
            ...
            throw new Error(
                'Jazzy-UI: unsupported `%s` color space.\n' +
 ...
 */
export const recomposeColor = (color: JazzyColor | string): string => {
    if (typeof color !== 'string') {
 ...
 */
export const hslToRgb = (color: JazzyColor): string => {
...
export const lighten = (color: string, coefficient: number): string => {
    ...
    if (colorObj.type.indexOf('hsl') !== -1) {
        ...
        for (let i = 0; i < 3; i += 1) {
            colorObj.values[i] +=
                (255 - colorObj.values[i]) * clampedCoefficient;
        }
